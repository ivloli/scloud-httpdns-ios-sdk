...
enum ResponseAdapter {
    ...
    static func parseResolvePayload(
        ...
        let ttl = max(1, ttlMapper?(requestHost, requestIpType, rawTtl) ?? rawTtl)
        ...
        let host = (answer?["dn"] as? String)?.isEmpty == false ? (answer?["dn"] as? String ?? requestHost) : requestHost
        let ipsV4 = (v4Obj?["ips"] as? [String] ?? []).filter { !$0.isEmpty }
        let ipsV6 = (v6Obj?["ips"] as? [String] ?? []).filter { !$0.isEmpty }

        return ResolveItem(host: host, ipsV4: ipsV4, ipsV6: ipsV6, ttl: ttl, extras: extras)
    }
